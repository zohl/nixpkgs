{ stdenv, runCommand, nettools, bc, perl, kmod, openssl, writeTextFile, ubootChooser, ncurses }:
...
stdenv.mkDerivation ((drvAttrs config stdenv.platform (kernelPatches ++ nativeKernelPatches) configfile) // {
  ...
  nativeBuildInputs = [ perl bc nettools openssl ncurses ] ++ optional (stdenv.platform.uboot != null)
